        ...
        SatVibMap.InputBegan:Connect(function(Input)
            if Library:IsPrimaryInput(Input) then
                while Library:IsPrimaryHeld(Input) do
                    ...
                    local MouseX = math.clamp(Library:GetPointerPosition().X, MinX, MaxX);

                    local MinY = SatVibMap.AbsolutePosition.Y;
                    local MaxY = MinY + SatVibMap.AbsoluteSize.Y;
                    local MouseY = math.clamp(Library:GetPointerPosition().Y, MinY, MaxY);
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local function RecalculateListSize(YSize)
            ListOuter.Size = UDim2.fromOffset(DropdownOuter.AbsoluteSize.X, YSize or (MAX_DROPDOWN_ITEMS * 22 + 2))
        end;
...
function Library:CreateWindow(...)
    ...
    local function ApplyResponsiveSize()
        ...
        local preferredW = (typeof(Config.Size) == 'UDim2' and Config.Size.X.Offset) or 460
        local preferredH = (typeof(Config.Size) == 'UDim2' and Config.Size.Y.Offset) or 540

        local minW = isTouch and 320 or 360
        local minH = isTouch and 380 or 420

        local maxW = math.floor(Safe.X * (isTouch and 0.82 or 0.55))
        local maxH = math.floor(Safe.Y * (isTouch and 0.78 or 0.80))
    ...
    local TabContainer = Library:Create('Frame', {
        BackgroundTransparency = 1; -- remove the extra "box" around tab content
        BorderSizePixel = 0;
    ...
    function Window:AddTab(Name)
        ...
        local ActiveUnderline = Library:Create('Frame', {
            BackgroundColor3 = Library.AccentColor;
            BorderSizePixel = 0;
        ...
        Library:AddToRegistry(ActiveUnderline, {
            BackgroundColor3 = 'AccentColor';
        });
